...
2. 从锁定的 pdf-lib 1.17.1 依赖复制 assets/vendor/pdf-lib.min.js。

`npm test` 会运行 17 项浏览器交互和 PDF 处理测试。测试夹具、分层与编写约定见 [TESTING.md](TESTING.md)；版本变化见 [CHANGELOG.md](CHANGELOG.md)，尚未完成的部署和 AdSense 工作见 [TODOS.md](TODOS.md)。
...
部署脚本会先 npm ci 和 npm run build，再上传一个新版本，最后原子切换 current 软链接：
...
- -SkipBuild：跳过本地构建，仅在已确认产物最新时使用。
- -SkipHealthCheck：跳过发布后的 URL 检查。
